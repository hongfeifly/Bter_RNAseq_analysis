...
# in terms of gene expression associated with the queen-biassed gene in fat body.
...
## Read in information
fq_score <- read.csv(file = "../GO_data/fatbody_gene_queen_score.csv")
...
topgo_genelist <- fq_score$allScore
names(topgo_genelist) <- fq_score$gene_id

## Get top 5% genes
quantile(topgo_genelist, 0.05)

topDiffGenes <- function(allScore){
  return(allScore <= 0.4585115)
}

sum(topDiffGenes(topgo_genelist))
# 569 genes here, fq_bias(495)
...
out_dir <- "fatbody_queen_results_20"
