...
# title: "**Gene Ontology term enrichment analysis of worker-biassed genes
# in fat body of _Bombus terrestris_**"
# output:
...
# in terms of gene expression associated with worker-biassed genes in fat body.
...
## Read in information
fw_score <- read.csv(file = "../GO_data/fatbody_gene_worker_score.csv")
...
topgo_genelist <- fw_score$allScore
names(topgo_genelist) <- fw_score$gene_id

## Get top 5% genes
quantile(topgo_genelist, 0.05)

topDiffGenes <- function(allScore){
  return(allScore <= 0.3846355)
}

sum(topDiffGenes(topgo_genelist))
# 569 genes here, fw_bias(392)

## Define node size:
node_size <- 20

## setup output directory
out_dir <- "fatbody_worker_results_20"
