...
# in terms of gene expression associated with the queen-biassed gene in RTs.
...
## Read in information
sq_score <- read.csv(file = "../GO_data/spermatheca_gene_queen_score.csv")
...
topgo_genelist <- sq_score$allScore
names(topgo_genelist) <- sq_score$gene_id

## Get top 5% genes
quantile(topgo_genelist, 0.05)

topDiffGenes <- function(allScore){
  return(allScore <= 0.3224039)
}
...
out_dir <- "spermatheca_queen_results_20"
