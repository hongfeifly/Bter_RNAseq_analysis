...
# in terms of gene expression associated with the worker-biassed genes in ovary.
...
## Read in information
ow_score <- read.csv(file = "../GO_data/ovary_gene_worker_score.csv")
...
topgo_genelist <- ow_score$allScore
names(topgo_genelist) <- ow_score$gene_id

## Get top 5% genes
quantile(topgo_genelist, 0.05)

topDiffGenes <- function(allScore){
  return(allScore <= 0.7004266)
}

sum(topDiffGenes(topgo_genelist))
# 579 genes here, fw_bias(133)

## Define node size:
node_size <- 20

## setup output directory
out_dir <- "ovary_worker_results_20"
