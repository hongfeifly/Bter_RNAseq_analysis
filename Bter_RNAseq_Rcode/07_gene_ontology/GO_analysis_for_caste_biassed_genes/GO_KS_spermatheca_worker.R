...
# in terms of gene expression associated with the worker-biassed genes in RTs.
...
## Read in information
sw_score <- read.csv(file = "../GO_data/spermatheca_gene_worker_score.csv")
...
topgo_genelist <- sw_score$allScore
names(topgo_genelist) <- sw_score$gene_id

## Get top 5% genes
quantile(topgo_genelist, 0.05)

topDiffGenes <- function(allScore){
  return(allScore <= 0.3978236)
}
...
out_dir <- "spermatheca_worker_results_20"
