...
# in brain of _Bombus terrestris_**"
# output:
...
# in terms of gene expression associated with the worker-biassed genes in brain.
...
## Read in information
bw_score <- read.csv(file = "../GO_data/brain_gene_worker_score.csv")
...
topgo_genelist <- bw_score$allScore
names(topgo_genelist) <- bw_score$gene_id

## Get top 5% genes
quantile(topgo_genelist, 0.05)

topDiffGenes <- function(allScore){
  return(allScore <= 0.3968639)
}

sum(topDiffGenes(topgo_genelist))
# 576 genes here, bw_bias(56)

## Define node size:
node_size <- 20

## setup output directory
out_dir <- "brain_worker_results_20"
