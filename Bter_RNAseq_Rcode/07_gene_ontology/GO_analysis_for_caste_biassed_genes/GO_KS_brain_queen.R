...
# in terms of gene expression associated with the queen-biassed gene in brain.
...
## Read in information
bq_score <- read.csv(file = "../GO_data/brain_gene_queen_score.csv")
...
topgo_genelist <- bq_score$allScore
names(topgo_genelist) <- bq_score$gene_id

## Get top 5% genes
quantile(topgo_genelist, 0.05)

topDiffGenes <- function(allScore){
  return(allScore <= 0.4641876)
}

sum(topDiffGenes(topgo_genelist))
# 576 genes here, sq_bias(159)

## Define node size:
node_size <- 20

## setup output directory
out_dir <- "brain_queen_results_20"
