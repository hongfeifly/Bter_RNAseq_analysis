...
# title: "**Transform spermatheca sample informatin for
...
samples_infor <- read.table(file = "../data/spermatheca_sample_information_star.txt",
                                  header = FALSE)
...
samples_information$treatment <- gsub(pattern = "Control",
  ...
  gsub(pattern = "Insemination",
       replacement = 1,
       samples_infor$treatment)
## make the data of samples information as numeric
...
write.csv(samples_information,
          file = "../data/spermatheca_sample_information_WGCNA.csv")
