...
# title: "**Weight gene co-expression network analysis of ovary of
...
data <- readRDS(file = "../data/vsd_ovary_caste.rds")
...
samples_information <- read.csv(file = "../data/ovary_sample_information_WGCNA.csv",
                                row.names = 1)

# Save WGCNA basic data for use in subsequent parts
save(transposed_data_filt,
     samples_information,
     file = "results/ovary_basic_data.RData")
...
pdf(file = "results/sample_dendrogram_trait_heatmap_ovary.pdf",
    height = 6,
    width = 15)
...
################################################################################
Connectivity=softConnectivity(transposed_data_filt,corFnc = "cor",
                              corOptions = "use ='p'",
                              power=softPower,
                              type = "signed")
...
sizeGrWindow(12, 9)
pdf(file = "results/geneDendro-ovary.pdf", wi = 9, he = 6)
...
save(exp_MEs,
     ...
     exp_moduleColors,
     exp_geneTree,
     file = "results/ovary-02-networkConstruction-stepByStep.RData")
...
pdf(file = "results/module-trait_relationship_heatmap_ovary.pdf",
    height = 6,
    width = 8)
par(mar = c(6, 8.5, 3, 3));
...
save(exp_MEs,
     ...
     samples_information,
     exp_textMatrix,
     file = "results/ovary-03-input_for_labelled_heatmap.RData")
...
save(exp_geneModuleMembership,
     ...
     file = "results/ovary-04-input_for_membership_scatterplot_brown.RData")

################################################################################
## Further analysis
## Load experimental data:
load(file = "results/ovary_basic_data.RData")
load(file = "results/ovary-02-networkConstruction-stepByStep.RData")
load(file = "results/ovary-04-input_for_membership_scatterplot_brown.RData")
...
o_brown_data <- exp_mm[exp_moduleGenes, c("gene_id", "MMbrown")] %>%
  left_join(caste_sig, by = "gene_id")

save(o_brown_data,
     file = "results/ovary_MM_GS_data.RData")
