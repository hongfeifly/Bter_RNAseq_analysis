...
# title: "**Weight gene co-expression network analysis of fat body of
...
setwd("E:/rna_seq_analysis/results/Advanced_analysis/WGCNA/fatbody")
getwd()
...
data <- readRDS(file = "../data/vsd_fatbody_caste.rds")
...
## For the experimental dataset, there is an issue with sample BtwC1b_F2:
...
samples_information <- read.csv(file = "../data/fatbody_sample_information_WGCNA.csv",
                                row.names = 1)

# Save WGCNA basic data for use in subsequent parts
save(transposed_data_filt,
     samples_information,
     file = "results/fatbody_basic_data.RData")
...
pdf(file = "results/sample_dendrogram_trait_heatmap_fatbody.pdf",
    height = 6,
    width = 15)
...
pdf("results/scale-free softpower.pdf");
...
sizeGrWindow(12, 9)
pdf(file = "results/geneDendro-fatbody.pdf", wi = 9, he = 6)
...
pdf(file = "results/module-trait_relationship_heatmap_fatbody.pdf",
    height = 6,
    width = 8)
par(mar = c(6, 8.5, 3, 3));
...
save(exp_geneModuleMembership,
     ...
     file = "results/fatbody-04-input_for_membership_scatterplot_black.RData")

################################################################################
## Further analysis
## Load experimental data:
load(file = "results/fatbody_basic_data.RData")
load(file = "results/fatbody-02-networkConstruction-stepByStep.RData")
load(file = "results/fatbody-04-input_for_membership_scatterplot_black.RData")
...
f_black_data <- exp_mm[exp_moduleGenes, c("gene_id", "MMblack")] %>%
  left_join(caste_sig, by = "gene_id")

exp_moduleGenes = exp_moduleColors == "magenta"
f_magenta_data <- exp_mm[exp_moduleGenes, c("gene_id", "MMmagenta")] %>%
  left_join(caste_sig, by = "gene_id")

exp_moduleGenes = exp_moduleColors == "lightgreen"
f_lgreen_data <- exp_mm[exp_moduleGenes, c("gene_id", "MMlightgreen")] %>%
  left_join(caste_sig, by = "gene_id")

exp_moduleGenes = exp_moduleColors == "red"
f_red_data <- exp_mm[exp_moduleGenes, c("gene_id", "MMred")] %>%
  left_join(caste_sig, by = "gene_id")

exp_moduleGenes = exp_moduleColors == "yellow"
f_yellow_data <- exp_mm[exp_moduleGenes, c("gene_id", "MMyellow")] %>%
  left_join(caste_sig, by = "gene_id")
...
save(f_black_data,
     ...
     f_red_data,
     f_yellow_data,
     file = "results/fatbody_MM_GS_data.RData")
